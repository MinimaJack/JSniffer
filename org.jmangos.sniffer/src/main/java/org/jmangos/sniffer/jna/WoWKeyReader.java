...
public class WoWKeyReader {
    ...
    static {
        if (Platform.isWindows()) {
            if (Platform.is64Bit()) {
                INSTANCE =
                        (Tools) Native.loadLibrary("win32-amd64", Tools.class,
                                W32APIOptions.UNICODE_OPTIONS);
            } else {
                INSTANCE =
                        (Tools) Native.loadLibrary("win32-x86", Tools.class,
                                W32APIOptions.UNICODE_OPTIONS);
            }
        } else {
    ...
    synchronized public byte[] getKey() {
        if (this.authKey == null) {
            ...
            final HANDLE wow =
                    com.sun.jna.platform.win32.Kernel32.INSTANCE.OpenProcess(
                            Kernel32.PROCESS_QUERY_INFORMATION | Kernel32.PROCESS_VM_READ, true,
                            INSTANCE.GetTargetProcessId("wow.exe"));

            if (wow != null) {
                final Memory connection = new Memory(4);
                int bytes =
                        INSTANCE.ReadMemory(wow, new Pointer(this.connectionOffset),
                                connection, (int) connection.size());
                final Memory key = new Memory(40);
                final Pointer array = new Pointer(connection.getInt(0) + this.keyOffset);
