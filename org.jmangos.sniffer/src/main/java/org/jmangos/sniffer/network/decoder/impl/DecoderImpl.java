 ...
 */
@Service
public class DecoderImpl implements Decoder, ApplicationContextAware {

    Logger log = LoggerFactory.getLogger(DecoderImpl.class);

    private final HashMap<String, NetworkChannel> activeChannels =
            new HashMap<String, NetworkChannel>();
    ...
    public void encode() {
        ...
        if (alldevs.isEmpty()) {
            this.log.error("Can't read list of devices, error is {}", errbuf.toString());
            return;
        }
        for (int i = 0; i < alldevs.size(); i++) {
            this.log.info("Network device: {} ", alldevs.get(i).getAddresses().get(0).getAddr());
        }
        ...
        log.info("Selected device: {}", alldevs.get(deviceIndex).getAddresses().get(0).getAddr());
        ...
        if (pcap == null) {
            this.log.error("Error while opening device for capture: {}", errbuf.toString());
            return;
        }
    ...
    public void onConnect(final String channelHash) {
        ...
        final NetworkChannel channel = this.context.getBean(NetworkChannel.class);
        ...
        if (pktEnabled == 1) {
            channel.addLogHandler(this.pktLogHandler);
        }
        if (txtEnabled == 1) {
            channel.addLogHandler(this.txtLogHandler);
        }
    ...
    public void onDisconnect(final String channelHash) {
        ...
        if (this.activeChannels.containsKey(channelHash)) {
            final NetworkChannel savedSession = this.activeChannels.get(channelHash);
            ...
            if (this.activeChannels.size() == 0) {
                for (final PacketLogHandler logger : savedSession.getPacketLoggers()) {
